...
var RecipeFinder =
{
  ...
  counting:function () {
    ...
    // count = recipesDB.count({cached:false},function (err,count) { RecipeFinder.count = count})
    // recipesDB.find({cached:false}).limit(-1).skip(Math.random() * count
  // recipesDB.findOne({cached:false}
    // ,function(err,recipeURL) {
  //     if (err) return console.error(err);
